...
from src.data.datasets import build_debug_dataset, build_dataloader
...
def main():
    ...
    # đưa các tham số này vào file config.yaml ở Giai đoạn 4
    ...
    dataloader = build_dataloader(
        ...
        shuffle=True,
        num_workers=2 #  Tăng tốc độ load ảnh nếu chạy trên Colab/GPU
    )
    ...
    for epoch in range(epochs):
        ...
        # 6. (Tùy chọn) Lưu checkpoint đơn giản
        checkpoint_path = Path("checkpoints")
        checkpoint_path.mkdir(exist_ok=True)
        torch.save(model.state_dict(), checkpoint_path / f"adain_epoch_{epoch+1}.pth")

    print("Hoàn thành quá trình huấn luyện! ck n.")
