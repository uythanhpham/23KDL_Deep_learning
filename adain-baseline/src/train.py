...
from adain_baseline.src.data.datasets import build_dataloaders, build_real_dataloaders
...
class EarlyStopping:
    ...
    def step(self, val_loss: float) -> bool:
        """Gọi sau mỗi epoch. Trả về True nếu nên dừng."""
...
def parse_args():
    ...
    parser.add_argument("--data_mode",      type=str,   default="real",
                        choices=["debug", "real"],
                        help="debug: dùng debug_data | real: dùng processed-data")
    parser.add_argument("--root_dir",       type=str,
                        default="/kaggle/input/datasets/tminhhongnguyn/processed-data",
                        help="real: đường dẫn tới processed-data/ | debug: đường dẫn tới debug_data/")
    parser.add_argument("--image_size",     type=int,   default=256)
    parser.add_argument("--batch_size",     type=int,   default=8)
    parser.add_argument("--num_workers",    type=int,   default=2)
    parser.add_argument("--val_split",      type=float, default=0.2,
                        help="Chỉ dùng khi data_mode=debug")
...
def main():
    ...
    print(f"--- Device: {device} | Mode: {args.data_mode} ---")

    # 1. Data — chọn loader theo data_mode
    if args.data_mode == "real":
        print(f"[Data] Load từ processed-data: {args.root_dir}")
        ...
        print(f"[Data] Load từ debug_data: {args.root_dir}")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, train_batch in enumerate(train_loader):
            ...
            # Val batch — reset iterator khi hết
            try:
            ...
            if (batch_idx + 1) % 10 == 0:
                print(
                    ...
                    f"| Train: {loss_dict['train_total_loss']:.4f} "
                    f"(C: {loss_dict['train_content_loss']:.4f}, S: {loss_dict['train_style_loss']:.4f}) "
                    f"| Val: {loss_dict['valid_total_loss']:.4f}"
                )

        # ── Tổng kết epoch ──
        avg_train_loss = epoch_train_loss / len(train_loader)
        avg_val_loss   = loss_dict["valid_total_loss"]

        print(
            f"==> Epoch {epoch+1} | "
            f"Avg Train: {avg_train_loss:.4f} | "
            f"Val (last step): {avg_val_loss:.4f}"
        )
