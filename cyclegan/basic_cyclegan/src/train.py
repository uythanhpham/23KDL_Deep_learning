...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train CycleGAN theo cấu trúc <root>/<style>/trainA/trainB"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def move_optimizer_to_device(optimizer: torch.optim.Optimizer, device: torch.device) -> None:
    """
    Khi resume checkpoint được load từ CPU/GPU khác, state của optimizer
    có thể nằm sai device. Hàm này chuyển tensor state về device hiện tại.
    """
...
def main() -> None:
    ...
    scheduler_G = torch.optim.lr_scheduler.LambdaLR(
        optim_G,
        lr_lambda=lambda epoch_index: lambda_lr(epoch_index, n_epochs, n_epochs_decay),
    )
    ...
    if args.resume is not None:
        ...
        if not resume_path.exists():
            raise FileNotFoundError(f"Không tìm thấy checkpoint resume: {resume_path}")
        ...
        if "scheduler_G" in ckpt:
            ...
            print("[Warning] Checkpoint không có scheduler_G. LR scheduler sẽ được khởi tạo lại.", flush=True)

        if "scheduler_D" in ckpt:
            scheduler_D.load_state_dict(ckpt["scheduler_D"])
        else:
            print("[Warning] Checkpoint không có scheduler_D. LR scheduler sẽ được khởi tạo lại.", flush=True)
        ...
        # Lưu ý:
        # Nếu checkpoint được lưu giữa epoch, code này vẫn resume từ epoch + 1 như bản cũ.
        # Nghĩa là nó không resume đúng step giữa epoch.
        # Nhưng trọng số/optimizer mới nhất vẫn được giữ.
        start_epoch = int(ckpt.get("epoch", 0)) + 1

        print(f"[Resume] checkpoint={resume_path}", flush=True)
        print(f"[Resume] tiếp tục từ epoch {start_epoch}/{total_epochs}", flush=True)
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for step, batch in enumerate(pbar, start=1):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                loss_G, out = model.compute_generator_loss(real_A, real_B)
            ...
            last_tensors = (real_A, real_B, out)

            # ---------------------------------------------------------
            # Lưu checkpoint giữa epoch để tránh mất toàn bộ epoch
        ...
        if last_tensors is not None and epoch % sample_every_epochs == 0:
            real_A, real_B, out = last_tensors
            ...
            print(f"[Save] sample: {sample_dir / f'epoch_{epoch:03d}.jpg'}", flush=True)
